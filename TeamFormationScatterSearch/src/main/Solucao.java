...
public class Solucao {
	...
	/**
	 * Heuristica gulosa ou aleatoria, ou os dois, que tenta caminhar pra mais
	 * perto do otimo.
	 * 
	 * @param instancia
	 *            instancia do problema com os herois disponiveis.
	 */
	public void melhoraSolucao(Instancia instancia) {
		...
		int menorInd = 0;
		int i;
		for (i = 0; i < listaDeHerois.size(); i++) {
			double valor = listaDeHerois.get(i).getValue(instancia);
			if (valor < menor) {
				menorInd = i;
				menor = valor;
			}
		...
		Personagem heroi;
		for (int j = 0; j < instancia.maxHeroIndex; j++) {
			boolean estaContido = false;
			heroi = instancia.personagens.get(i);
			for (Personagem q : listaDeHerois) {
				if (q.getId() == heroi.getId()) {
					estaContido = true;
				}
			}
			if (!estaContido && heroi.getValue(instancia) > menor) {
				listaDeHerois.remove(listaDeHerois.get(menorInd));
	...
	/**
	 * Recebe pelo menos duas solucoes, e gera pelo menos uma outra. Essa funcao
	 * tem que combinar as solucoes de maneira nao aleatoria.
	 * 
	 * @param solucoes
	 *            conjunto de solucoes que sera combinado.
	...
	public static List<Solucao> geraNovasSolucoes(List<Solucao> solucoes,
		...
		// Do jeito que implementei, ele combina de todas as maneiras possiveis,
		// sem muita inteligencia.
		...
		for (int i = 0; i < solucoes.size(); i++) {
			for (int j = i + 1; j < solucoes.size(); j++) {
				...
				for (int k = 0; k < Math.max(s.listaDeHerois.size(),
						t.listaDeHerois.size()); k++) {
					Solucao st = new Solucao();
					Solucao ts = new Solucao();
					for (int l = 0; l < k; l++) {
						if (l < s.listaDeHerois.size()
								&& !st.listaDeHerois.contains(s.listaDeHerois
										.get(l))) {
							st.listaDeHerois.add(s.listaDeHerois.get(l));
						}
						if (l < t.listaDeHerois.size()
								&& !ts.listaDeHerois.contains(t.listaDeHerois
										.get(l))) {
							ts.listaDeHerois.add(t.listaDeHerois.get(l));
						}
					}
					for (int l = k; l < Math.max(s.listaDeHerois.size(),
							t.listaDeHerois.size()); l++) {
						if (l < t.listaDeHerois.size()
								&& !st.listaDeHerois.contains(t.listaDeHerois
										.get(l))) {
							st.listaDeHerois.add(t.listaDeHerois.get(l));
						}
						if (l < s.listaDeHerois.size()
								&& !ts.listaDeHerois.contains(s.listaDeHerois
										.get(l))) {
	...
	private double calculaBudget(Instancia instancia) {
		...
		exp2 = factor * powerGridHerois * popularidadeHerois
				* instancia.viloes.length;
	 ...
	 * 
	 * @param instancia
	 *            uma instancia do problema.
	 * @return <code>true</code> se for factivel, <code>false</code> caso
	 *         contrario.
	 */
	public boolean factível(Instancia instancia) {
		return (listaDeHerois.size() >= instancia.minHerois
				&& listaDeHerois.size() <= instancia.maxHerois && calculaBudget(instancia) < instancia.budget);
	}

	/**
	 * Determina se o conjunto de herois da solucao eh capaz de derrotar os
	 * viloes.
	 * 
	 * @param instancia
	 *            .
	 ...
	 * arestas entre todos os herois da solucao mais as arestas e entre os
	 * herois da solocao e os viloes da instancia.
	 * 
	...
	public boolean equals(Object obj) {
		...
		if (obj.getClass().equals(getClass())) {
			...
			if (listaDeHerois.size() == l.listaDeHerois.size()) {
				for (int i = 0; i < listaDeHerois.size(); i++) {
					if (listaDeHerois.get(i).getId() != l.listaDeHerois.get(i)
							.getId()) {
						sameIds = false;
						break;
					}
